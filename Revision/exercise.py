...
mydict = {"a":1, "b":6, "c":10, "e":14, "d":18}

#find the largest key, key with the highest value and the highest value

largest_key = max(mydict)
print(largest_key)
key_with = max(mydict, key = lambda key:mydict[key])
print(key_with)
highest_value = mydict[key_with]
print(highest_value)

names = ["Rishikesh", "aman", "Ajay", "Hemkesh", "sandeep", "Darshan", "Virendra", "Shwetabh"]
print(sorted(names))
#but we want all alphabets to be togrther no matter what
newnames = sorted(names, key=lambda name:name.lower())
...
value = (max(second_dict, key = lambda key:second_dict[key]))

print((second_dict[value]))
...
def palindrome():
    ...
    if y == rev:
        ...
        print("this is not a palindrome num")
